...
session_start(); $option = (isset($_GET['option'])) ? $_GET['option'] : "part1"; ?>
<html>
...
<body>
	...
	<div>
		...
		<div>
			...
			<?php if (isset($_SESSION['ans']) && isset($_SESSION["ans_num"]) && $_SESSION["ans_num"] == "part1") { ?>
			<table>
				<thead><tr><th><b>Author Name</b></th></tr></thead>
				<tbody><?php
				foreach ($_SESSION['ans'] as $obj)
				{
					print "<tr><td>$obj[name]</td></tr>";
				}
			    ?></tbody>
			</table>
	...
	<div>
		...
		<div>
			<form action="backend.php" method="POST">
				<input type="text" name="cust_name" placeholder="Name of customer" />
			...
			<?php if (isset($_SESSION['ans']) && isset($_SESSION["ans_num"]) && $_SESSION["ans_num"] == "part3") { 
				...
				foreach ($_SESSION['ans'] as $obj)
				{
					...
					print "On $datetime, <b>$_SESSION[cust_name]</b> bought <b>$obj[title]</b> for <b>$$obj[price]</b>.<br/>";
				}
	...
	<div>
		...
		<div>
			...
			<table>
				<thead><tr><th><b>ISBN13</b></th><th><b>Title</b></th><th><b>Year</b></th><th><b>Category</b></th><th><b>Price</b></th><th><b>Author name</b></th><th><b>Author address</b></th><th><b>Author SSN</b></th><th><b>Publisher name</b></th><th><b>Publisher city</b></th><th><b>Publisher state</b></th></tr></thead>
				<tbody><?php
				foreach ($_SESSION['ans'] as $obj)
				{
					$ISBN = $obj -> getISBN();
					...
					$price = $obj -> getPrice();
					$author_list = $obj -> getAuthor();
					$publisher_name = $obj -> getPublisher() -> getName();
					$publisher_city = $obj -> getPublisher() -> getCity();
					$publisher_state = $obj -> getPublisher() -> getState();
					for ($i = 0; $i < count($author_list); $i++) {
						$author_name = $author_list[$i] -> getName();
						$author_address = $author_list[$i] -> getAddress();
						$author_SSN = $author_list[$i] -> getSSN();
						if ($i == 0)
							print "<tr><td>$ISBN</td><td>$title</td><td>$year</td><td>$category</td><td>$price</td><td>$author_name</td><td>$author_address</td><td>$author_SSN</td><td>$publisher_name</td><td>$publisher_city</td><td>$publisher_state</td></tr>";
						else
							print "<tr><td></td><td></td><td></td><td></td><td></td><td>$author_name</td><td>$author_address</td><td>$author_SSN</td><td></td><td></td><td></td></tr>";
					}
				}
			} unset($_SESSION['ans']); unset($_SESSION['ans_num']); ?>
		</div>
