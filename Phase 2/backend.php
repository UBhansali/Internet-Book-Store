...
function doPartThree($conn, $name) {
	try {
		$stmt = $conn->prepare("SELECT * FROM book b, purchase p WHERE cid = (SELECT cid FROM Customer WHERE name = :name) AND p.ISBN13 = b.ISBN13");
		$stmt->execute(array(':name' => $name));
		return $stmt->fetchAll();
	} catch (Exception $e) {
		return -1;
...
function doPartFour($conn, $keyword) {
	try {
		$stmt = $conn->prepare('SELECT b.ISBN13, b.title, b.year, b.category, b.price, a.name, a.address, a.ssn, p.pname, p.city, p.state FROM book b, author a, publisher p, writes w WHERE b.title LIKE :keyword AND w.ISBN13 = b.ISBN13 AND a.aid = w.aid AND b.pname = p.pname');
		...
		foreach ($results as $row) {
			$author = new Author($row["name"], $row["address"], $row["ssn"]);
			$publisher = new Publisher($row["pname"], $row["city"], $row["state"]);
			if ($row["ISBN13"] == $curr_ISBN) { // same book, so add author
				$book_author = $curr_book -> $author;
				$book_author[] = $author;
				$book = new Book($row["ISBN13"], $row["title"], $row["year"], $row["category"], $row["price"], $book_author, $publisher);
				$curr_book = $book;
			} else {
				$curr_ISBN = $row["ISBN13"];
				$book = new Book($row["ISBN13"], $row["title"], $row["year"], $row["category"], $row["price"], $author, $publisher);
				$curr_book = $book;
			}
		}
		return $book;
...
header("Location: ./guest.php?option=" . $_SESSION['ans_num']);
?>